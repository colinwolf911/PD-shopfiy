/* USF file - Do not modify this file since it is regularly changed. Auto modified at: 5/24/2021 11:05:57 AM*/
/* Begin custom theme code */
// define templates for the Porto theme
...
var _usfSearchResultsSkeletonItemTpl = `
<div v-if="view === 'grid'" class="item product product-col product-default product-type-simple usf-skeleton">
    <div class="grid-view-item" v-if="true">
        <div class="usf-img"></div>
        <div class="usf-meta">            
        </div>
...
<div class="usf-sr-container" :class="{'usf-no-facets': noFacets, 'usf-empty': !loader && !hasResults, 'usf-nosearch': !showSearchBox}">
    ...
    <div :class="(view === \'grid\' ? \'category-products products grid products-spacing-7 item-1200-4 item-992-4 item-768-3 item-640-2 item-320-1\' : \'category-products products list\') + \' usf-\' + view">
    ...
    <div class="product-inner">
        ...
        <div class="product-image">
            <a :href="productUrl" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave">
                <div class="labels">
                    <div v-if="hasDiscount && usf.settings.search.showSale" class="onsale" v-html="loc.sale"></div>
                    <div v-if="isSoldOut && usf.settings.search.showSoldOut" class="onsale cat-label-label1" v-html="loc.soldOut"></div>
                </div>
                <div class="inner" :class="{'img-effect':product.images.length>1}"> 
                    <img class="img-responsive main lazyload" :data-src="getImgWithSize(scaledSelectedImageUrl,(getImageSizeString(selectedImage)['image_size']+'_crop_center'))" v-bind:style="'padding-top:'+getImageSizeString(selectedImage)['ratio_string']+'%;'"> 
                    <img class="hover-image img-responsive lazyload" :data-src="getImgWithSize(scaledHoverImageUrl,(getImageSizeString(selectedImage)['image_size']+'_crop_center'))" v-if="scaledHoverImageUrl" v-bind:style="'padding-top:'+getImageSizeString(hoverImage)['ratio_string']+'%;'">                
                </div>
                <!-- product image extra -->
            ...
            <div class="links-on-image">
                <div class="add-links-wrap">
                    <div class="add-links clearfix">
                        <a :href="_usfAddQuery(productUrl,'view=quickview')" class="action-list quickview-icon quickview"> 
                            <span data-translate="collections.general.quickview" v-html="loc.quickView"></span> 
                        </a>
                    </div>
                </div>
            </div>
        ...
        <div class="product-content">
            ...
            <!-- product title -->
            <a class="product-loop-title" :href="productUrl">
                <h3 class="shopify-loop-product__title"> 
                    <span class="lang1" :attrs="usf.plugins.invoke('getProductTitleAttrs', pluginData)" v-html="product.title"></span> 
                </h3> 
            </a>
            <!-- Product review -->
            ...
            <div class="description">
                <div class="lang1" v-html="product.description"></div>
            </div>
            <!-- price -->
            ...
            <span class="price">
                <span class="price-wrapper">
                    <template v-if="hasDiscount">
                        <del><span class="shopify-Price-amount amount"><span class="money" v-html="displayPrice"></span></span></del> 
                        <ins><span class="shopify-Price-amount amount"><span class="money" v-html="displayDiscountedPrice"></span></span></ins>
                    </template>
                    <template v-else>
                        <span class="shopify-Price-amount amount"><span class="money" v-html="displayPrice"></span></span>
                    </template>
                </span>
            </span>

            <usf_swatch :product="product"></usf_swatch>

            <div class="add-links-wrap">
                <div class="add-links clearfix">
                    <a :href="productUrl" class="button product_type_variable add_to_cart_button" v-if="product.variants.length>1">
                        <span data-translate="products.product.add_to_cart" v-html="loc.addToCart"></span>
                    </a>
                    <form :action="usf.platform.baseUrl + '/cart/add'" method="post" enctype="multipart/form-data" v-else>
                        <select class="d-none" name="id">
                            <option :value="variant.id" v-for="variant in product.variants">{{product.title}} - {{ variant.price }}</option>
                        </select>
                    ...
                    <div class="yith-wcwl-add-to-wishlist">
                        <div class="yith-wcwl-add-button show">
                            <div :class="'product-wishlist wishlist-'+product.id"> 
                                <a href="javascript:;" :data-product-handle="product.urlName" :data-product-title="product.title" class="add_to_wishlist link-wishlist" title="Add to wishlist">
                                    <span data-translate="wish_list.general.add_to_wishlist">Add to wishlist</span>
                                </a>
                            </div>
                        </div>
                    </div>
                    <div class="clear"></div>
                    <a href="javascript:;" :data-product-handle="product.urlName" :data-product-title="product.title" class="link-compare"> 
...
<div class="pagination text-center">
    <template v-for="e in elements">
        <span v-if="e.type === 'prev'" class="prev">
            <a href="javascript:void(0)" :title="loc.prevPage" @click="onPrev">?</a>
        </span>
        ...
        <span v-else-if="e.type === 'next'" class="next" style="padding:0 7px">
            <a href="javascript:void(0)" :title="loc.nextPage" @click="onNext">?</a>
        </span>
    </template>
</div>
`,
...
var getImgWithSize = function (img, size = '') {
    return img.replace('{size}x', size)
}

function getImageSizeString(i) {
    var image_width, image_height;
    if (_usfSettingGlobal.collection_aspect_ratio == 1) {
        image_width = _usfSettingGlobal.collection_ratio_width;
        image_height = ''
        ar = 1
    } else {
        image_width = _usfSettingGlobal.collection_ratio_width;
        image_height = _usfSettingGlobal.collection_ratio_height;
    ...
    var image_size = `${image_width}x${image_height}`
    var ratio = 100 / ar;
    var ratio_string = ratio
    /*console.log({
        image_size,
        ratio_string
    })*/
...
usf.event.add('init', function () {
    ...
    for (var i = 0; i < nodes.length; i++) {
        var n = nodes[i];
        if (n.href && (n.href.indexOf('styles.scss.css') !== -1 || n.href.indexOf('usf.js') !== -1)) {
            usfFilesUrl = n.href;
    ...
    document.querySelectorAll(".search-results-wrapper").forEach(e => e.parentNode.removeChild(e));
    ...
    var usf_swatch = {
        ...
        render(h) {
            var opts = this.product.options;
            var variants = this.product.variants;
            var images = this.product.images;
            if (_usfSettingGlobal.collection_grid_swatch_type == '1' || _usfSettingGlobal.collection_grid_swatch_type == '2') {
                return h('div', {
                        ...
                        opts.map((o, index) => {
                            if (o.name.toLowerCase().includes('color')) {
                                ...
                                return variants.map((v) => {
                                    var color = o.values[v['options'][index]];
                                    var c_mod = _usfHandlezie(color)
                                    var image = images[v['imageIndex']]
                                    if (!colorlist.includes(color)) {
                                        colorlist.push(color)
                                        return h('li', [
                                            (_usfSettingGlobal.collection_grid_swatch_type == '2' && image) ?
                                                h('a', {
                                                    staticClass: 'filter-item filter-image',
                                                    attrs: {
                                                        href: 'javascript:void(0);',
                                                        'data-image': _usf_getIMGBySizeFromRawURL(image.url, '600x_crop_center')
                                                    },
                                                    style: {
                                                        backgroundImage: `url('${_usf_getIMGBySizeFromRawURL(image.url, '50x50')}')`
                                                    }
                                                }, [
                                                    h('i', {
                                                        staticClass: 'image-swatch',
                                                        style: {
                                                            backgroundImage: `url('${_usf_getIMGBySizeFromRawURL(image.url, '50x50')}')`
                                                        }
                                                    })
                                                    ...
                                                    image ?
                                                        h('a', {
                                                            ...
                                                            style: {
                                                                backgroundImage: `url('${usfFilesUrl}${c_mod}.png')`
                                                            }
                                                        }, [
                                                            h('i', {
                                                                staticClass: 'image-swatch',
    ...
    usf.register(usf_swatch, null, "usf_swatch");
});
...
$(document).ready(function () {
    ...
    $(document).on("click", ".quickview", function (e) {
        ...
        var url = $(this).attr("href"),
            item = $(this);
        newQuickViewLoad(url, item);
    });
    $(document).on('mouseenter', '#usf_container .configurable-swatch-list li a', function (e) {
        e.preventDefault();
    ...
    $(document).on('mouseenter', '#usf_container .filter-item-list li a', function (e) {
        e.preventDefault();
...
// fix auto add variant on url
function newQuickViewLoad(url, el) {
    if (!window.$) return;
    $("#resultLoading").show();
    $.ajax({
        url: url,
        ...
        success: function (data) {
            data = data.replace('enableHistoryState: true', 'enableHistoryState: false')
            $.magnificPopup.open({
                items: {
